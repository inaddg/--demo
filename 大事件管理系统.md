...
![76379335140](C:\Users\Amelia\AppData\Local\Temp\1763793351408.png)
...
// createRouter 创建路由实例
// 配置 history 模式
// 1. history模式(地址栏不带 #)：createWebHistory
// 2. hash模式(地址栏带 #)：createWebHashHistory
...
![76396963366](C:\Users\Amelia\AppData\Local\Temp\1763969633668.png)
...
​	EL组件官网：https://element-plus.org/zh-CN/component/layout
...
![76405300919](C:\Users\Amelia\AppData\Local\Temp\1764053009193.png)

##### ![76405211899](C:\Users\Amelia\AppData\Local\Temp\1764052118990.png)
...
<el-form :rules="rules" >
    <!--
 	// 整个表单的校验规则
    // 1. 非空校验 required: true      message消息提示，  trigger触发校验的时机： blur(鼠标失焦时触发) change（值改变的时候触发）
...
2. 注册之前进行校验（// 注册成功之前，先进行校验，校验成功 → 请求，校验失败 → 自动提示）
...
<template>
    ...
    el-menu-item 菜单项
      index="/article/channel" 配置的是访问的跳转路径，配合default-active的 值，实现高亮
  -->
...
<el-form inline>
  <el-form-item label="文章分类：">
     <!-- label 展示给用户看的， value 收集起来提交给后台的id -->
    <el-select>
...
```jsx
//table里面每一列的内容可以通过两种方法配置
// 1.prop 直接通过el-table-column 里面的prop
// 2.作用域插槽（官方文档table组件-自定义列模板）
...
### 上传文件 [element-plus - 文件预览(URL.createObjectURL)]

(URL.createObjectURL) => **临时照片查看链接生成器**
...
const onUploadFile = (uploadFile) => {
  // 预览图片语法：URL.createObjectURL(...) 创建本地预览的地址，来预览
...
3. 页面中使用绑定           => v-model:content="formModel.content"

   content-type="html"   =>  编辑器会将用户输入的富文本内容（比如字体、颜色、排版等	样式），以 HTML 代码的形式存储到绑定的`formModel.content`变量中；同时，当`formModel.content`里有 HTML 内容时，编辑器也会解析成对应的富文本样式展示出来。
...
1. ChatGPT 3.5 的使用   (4.0 使用方式一致，回答准确度更高，但付费，且每3小时，有次数限制)
   ...
   2. 三方整合产品
      ...
      - https://github.com/LiLittleCat/awesome-free-chatgpt（谷歌已收藏）
...
话术参考：
基于以下的代码给我在style中生成css样式，要求：
1.上传图片框为1px虚线，圆角，正方形，宽高250px左右
2.鼠标悬停在框上时，虚线框变为淡蓝色，且鼠标为一个小手
3.上传图片的加号在框中间
4.超过框大小的图片部分隐藏
5.其他部分自行发挥
...
注：这里按钮 button-选择图片， 想获取组件el-upload一样的效果（即点击图片后可以在文件中选取图片），则需获取组件，可以用（这里的input就是el-upload内部的原生文件上传输入框，通过点击它来触发选择文件的操作）：  @click="uploadRef.**$el.querySelector**('input').click()"

**头像选择预览**--（函数 onSelectFile ）

![76484067818](C:\Users\Amelia\AppData\Local\Temp\1764840678189.png)

方法1：` readAsDataURL`这个方法的**核心作用就是把文件内容转成 base64 编码的字符串**，

对**本地小文件**（**图片**、txt、pdf ）的 base64 转换，`readAsDataURL`是很常用的方法

![76484094813](C:\Users\Amelia\AppData\Local\Temp\1764840948133.png)
...
注：如果想要输入框小一点可以用以下代码包一下（12份=>占二分之一）:（或者直接给里面的input设宽高也行）
